#!/bin/bash
#SBATCH --job-name=prepare_dataset_llama_once
#SBATCH --output=logs/out/R-%x/R-%x_%A.out
#SBATCH --error=logs/error/R-%x/R-%x_%A.err
#
#SBATCH --nodes=1               # Request 2 nodes (1 GPU each)
...
HF_USERNAME="Siqi-Hu"
HF_REPO_NAME="Llama3-8B-generic-predictions-starwars"
HF_REPO_ID="${HF_USERNAME}/${HF_REPO_NAME}-bootstrap-coef-${BOOTSTRAP_COEF}-once"
...
srun python src/prepare_dataset_once.py \
    --context_length 128 \
